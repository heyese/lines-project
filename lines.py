...
def moves_list_special(total,max):
...
def moves_list(board, max_adj):
  moves = []
  explicit_part = explicit_partition(board)    # Get the explicit partition
  for (start, length) in explicit_part:
    ...
    temp_moves = moves_list_special(length, max_adj)
...
def take_turn(move_taken,moves):
...
def list_games(moves_list):
  ...
  if len(remaining_moves) <= 1:
    ...
    for move in remaining_moves:
      for game in list_games(take_turn(move,remaining_moves)):
        new_list = [ move ]
